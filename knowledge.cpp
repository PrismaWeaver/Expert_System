...
class Knowledge {
    ...
    std::vector<std::string> conlusionList = {/*a list of all automobile problem causes*/}; //conclusion list
    ...
    std::vector<std::vector<int>> clauseList = {/*an int vector of the above int vectors*/}; //clause list
    ...
    std::vector<int> CVL = CVLfill(clauseList);   //clause variable list
    ...
    std::vector<int> CVLfill(std::vector<std::vector<int>> clauseList) {
        int clauseNum = clauseList.size(), ref;
        std::vector<int> CVL(ruleNum * clauseNum);
        for (int i = 0; i < clauseNum; i++) {
            ref = i * ruleNum;
            for (int varsIndex : clauseList[i]) {
                CVL[ref + varsIndex] = -1;
            }
        }
        return CVL;
